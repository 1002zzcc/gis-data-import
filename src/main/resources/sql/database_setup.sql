-- 创建数据库
CREATE DATABASE IF NOT EXISTS gisdb CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- 确保geo_features表存在（如果不存在则创建）
CREATE TABLE IF NOT EXISTS `geo_features` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `feature_id` varchar(255) DEFAULT NULL COMMENT '要素ID',
  `geometry` longtext COMMENT '几何数据（WKT格式）',
  `attributes` longtext COMMENT '属性数据（JSON格式）',
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_feature_id` (`feature_id`),
  KEY `idx_created_at` (`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='地理要素数据表';
