...
public class ZbzhUtil {
    ...
    public static String crateLine(List<Map<String,Double>> list){
        if(list != null && list.size() > 0){
            ...
            for(int i = 0;i < list.size(); i++){
                ...
                Coordinate coordinate = new Coordinate(stringDoubleMap.get("x"), stringDoubleMap.get("y"));
     ...
     * 坐标转换主方法
     */
    public static List<String> CoordConvert2D(ZbzhDTO dto) throws IOException {
        ...
        String csName1 = dto.getSource();
        String csName2 = dto.getTarget();
    ...
    public static Map<String,Object> GetCoordSystem() throws IOException {
        org.springframework.core.io.Resource resource = new ClassPathResource("CoordSystem.json");
        return JsonUtil.readJson(resource.getInputStream(), Map.class, StandardCharsets.UTF_8);
    }
        ...
        // 如果不是同一个大地坐标系
            ...
            if (planeFourParam != null) {
                ...
                if (!coordConvertParam.getIsBLCoord1()) {
                    ...
                    if (!coordConvertParam.getL1().equals(L1)) {
                        double[] array = XY2BL(ellipsoid1, coordConvertParam.getL1(), X1, Y1);
                        double b1 = array[0];
                        double l1 = array[1];
                ...
                // 通过平面四参数进行转换
                if (isForward) {
                    double[] array = CoordConvert2DWithFourParam(planeFourParam, x1, y1);
                    x2 = array[0];
                    y2 = array[1];
                }
                else {
                ...
                // 如果目标坐标系不是经纬度
                if (!coordConvertParam.getIsBLCoord2()) {
                    if (coordConvertParam.getL2().equals(L2)) {
                        X2 = x2;
                        Y2 = y2;
                    }
                    else {
    ...
    public static double[] XY2BL(Ellipsoid ellipsoid, double L0, double X, double Y){
        ...
        double L = L0 + (D - (1 + 2 * T + C) * D * D * D / 6 + (5 - 2 * C + 28 * T - 3 * C * C + 8 * ee + 24 * T * T) * D * D * D * D * D / 120) / Math.cos(fai);
        double B = fai - (NN * Math.tan(fai) / R) * (D * D / 2 - (5 + 3 * T + 10 * C - 4 * C * C - 9 * ee) * D * D * D * D / 24 + (61 + 90 * T + 298 * C + 45 * T * T - 256 * ee - 3 * C * C) * D * D * D * D * D * D / 720);
    ...
    public static double[] BL2XY(Ellipsoid ellipsoid, double L0, double B, double L){
        ...
        double X = M + NN * Math.tan(B) * (A * A / 2 + (5 - T + 9 * C + 4 * C * C) * A * A * A * A / 24 + (61 - 58 * T + T * T + 600 * C - 330 * ee) * A * A * A * A * A * A / 720);
        double Y = NN * (A + (1 - T + C) * A * A * A / 6 + (5 - 18 * T + T * T + 72 * C - 58 * ee) * A * A * A * A * A / 120) + 500000L;
        result[0] = X;
        result[1] = Y;
        return result;
    }
     ...
     * 平面四参数正向转换
     */
    public static double[] CoordConvert2DWithFourParam(PlaneFourParam param, double X1, double Y1) {
        double[] result = new double[2];
        double X2 = 0;
        double Y2 = 0;
    ...
    public static double[] ReverseCoordConvert2D(PlaneFourParam param, double X1, double Y1) {
        double[] result = new double[2];
        double alfa = param.getAlfa() / 180.0 * Math.PI;
        double X2 = ((X1 - param.getDX()) * Math.cos(alfa) - (Y1 - param.getDY()) * Math.sin(alfa)) / (1 + param.getK());
        double Y2 = ((X1 - param.getDX()) * Math.sin(alfa) + (Y1 - param.getDY()) * Math.cos(alfa)) / (1 + param.getK());
        result[0] = X2;
        result[1] = Y2;
        return result;
    }
